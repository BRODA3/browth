...
export const PLANTILLAS: { id: string; nombre: string }[] = [
  { id: "capturar", nombre: "Capturar — del mensaje al CRM" },
  { id: "calificar", nombre: "Calificar — las tres preguntas" },
  { id: "convertir", nombre: "Convertir — del calificado al cierre" },
];

export function desdePlantilla(capaId: string, nombre: string): Workflow {
  const flujo = FLUJOS[capaId];
